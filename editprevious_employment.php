...
<body>
  ...
  if (isset($_POST['employer']) && isset($_POST['address'])
    ...
    $pi = (int)$_POST['previous_employment_id'];
    ...
    if($_POST['employer'] == '' || $_POST['address'] == ''
      ...
      || $_POST['end_date'] == '' || $_POST['bonus'] == '') {
        ...
        header("Location: editprevious_employment.php?borrower_id=$bi&previous_employment_id=$pi");
        return;
      }
    $sql = "UPDATE previous_employer SET employer = :employer,
      address = :address, income_source = :income_source,
      occupation = :occupation, start_date = :start_date, end_date = :end_date,
      income = :income, bonus = :bonus, type = :type, frequency = :frequency,
      borrower_id = :borrower_id WHERE borrower_id = :borrower_id AND
      previous_employment_id = :previous_employment_id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
  ...
  $sql = "SELECT * FROM previous_employer WHERE borrower_id = :borrower_id AND
  previous_employment_id = :previous_employment_id";
  $stmt = $pdo->prepare($sql);
  $stmt->execute(array(
    ":borrower_id" => $_GET['borrower_id'],
    ":previous_employment_id" => $_GET['previous_employment_id'],
  ));
  ...
  <div class="centered-edit">
    <?php echo "<h2>Edit $first_name's previous employment information</h2>"; ?>
  <form class="left-form" method="post">
    ...
    <input type="hidden" name="previous_employment_id"
      value="<?= htmlentities($_GET['previous_employment_id']) ?>">
    <p class="center-p">
