...
<body>
  ...
  if (isset($_POST['dob']) && isset($_POST['marital_status'])
    ...
    if ($_POST['own_rent'] == 'Rent') {
      if ($_POST['rent_amount'] == '') {
        ...
        header("Location: editborrower.php?borrower_id=$bi");
        return;
      };
    };
    if ($_POST['sin'] == '') {
    ...
    $sql = "UPDATE borrower_info SET DOB = :DOB, SIN = :SIN,
      marital_status = :marital_status, dependents = :dependents,
      home_phone = :home_phone, cell_phone = :cell_phone,
      work_phone = :work_phone, email = :email, address = :address,
      move_in_date = :move_in_date, rent_own = :rent_own,
      rent_amount = :rent_amount, previous_address = :previous_address,
  ...
  $rent = false;
  if ($row['rent_own'] == 'Rent') {
    $rent = true;
  };
  $previous_address = false;
  if (! empty($row['previous_address'])) {
    $previous_address = true;
  };

  $dob = htmlentities($row['DOB']);
  $sin = htmlentities($row['SIN']);
  $ms = htmlentities($row['marital_status']);
  $d = htmlentities($row['dependents']);
  $hp = htmlentities($row['home_phone']);
  $cp = htmlentities($row['cell_phone']);
  $wp = htmlentities($row['work_phone']);
  $e = htmlentities($row['email']);
  $a = htmlentities($row['address']);
  $mid = htmlentities($row['move_in_date']);
  $or = htmlentities($row['rent_own']);
  $ra = htmlentities($row['rent_amount']);
  $pa = htmlentities($row['previous_address']);
  $id = htmlentities($row['borrower_id']);
  ?>

  <div class="centered-edit">
    <?php echo "<h2>Edit $first_name's information</h2>"; ?>
  <form class="left-form" method="post">
    ...
    <p>
      ...
      <input type="number" id="sin" name="sin" value="<?php echo $sin ?>"
        ...
        if (move_date > date) {
          ...
          $('#previous_address').append(
            '<textarea name="previous_address" rows="12" \
              ...
              Dates You Lived There"><?php echo $pa ?></textarea>').show();
        }
      });
    });
  </script>
        ...
        if(selected_option == 'Rent') {
          ...
          $('#rent_amount').append(
            '<input type="number" id="rent_amount" \
              name="rent_amount" value="<?php echo $ra ?>">').show();
          };
        if(selected_option !== 'Rent') {
          $('#rent_amount').empty()
          }
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      $('#marital_status').val('<?php echo $ms ?>').change();
      $('#own_rent').val('<?php echo $or ?>').change();
    });
  </script>
