...
<body>
  ...
  <?php echo "<h2>$first_name's assets</h2>"; ?>
  <table>
    <tbody id="mytab">
    </tbody>
  </table>
  ...
  <script type="text/javascript">
  $.getJSON('getassets.php', function(rows) {
      $("#mytab").empty();
      found = false;
      for (var i = 0; i < rows.length; i++) {
          row = rows[i];
          found = true;
          $("#mytab").append('<h3>' + 'Asset # ' + (i + 1) + '</h3>');
          if (row.asset !== null) {
            $("#mytab").append('<tr><td>'
            + 'Asset</td><td>'
            + htmlentities(row.asset)
            + '</td></tr>');
          }
          if (row.detail !== null) {
            $("#mytab").append('<tr><td>'
          ...
          $("#mytab").append("<div class='buttons'><a class='edit-button' href='editassets.php?borrower_id="
          + htmlentities(row.borrower_id) + "&asset_id="
          + htmlentities(row.asset_id) +"'>Edit</a>"
          + ' '
          + "<a class='delete-button' href='deleteassets.php?borrower_id="
          + htmlentities(row.borrower_id) + "&asset_id="
          + htmlentities(row.asset_id) +"'>Delete</a></div>")
        }
      ...
      $("#options").empty();
      $("#options").append("<a class='button' href='addasset.php?borrower_id=<?=$_GET['borrower_id']?>'>Add</a>");
  });
  </script>
