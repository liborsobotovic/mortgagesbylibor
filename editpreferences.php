...
<body>
  ...
  if (isset($_POST['type']) && isset($_POST['product'])
    ...
    if ($_POST['type'] == '---SELECT---' || $_POST['product'] == '---SELECT---'
      ...
      || $_POST['source'] == '---SELECT---') {
        ...
        header("Location: editpreferences.php?borrower_id=$bi");
        return;
      };
    $sql = "UPDATE preferences SET type = :type, product = :product,
      price = :price, closing_date = :closing_date,
      mortgage_amount = :mortgage_amount, frequency = :frequency,
      amortization = :amortization, source = :source, borrower_id = :borrower_id
  ...
  $p = htmlentities($row['product']);
  $pr = htmlentities($row['price']);
  $cd = htmlentities($row['closing_date']);
  $ma = htmlentities($row['mortgage_amount']);
  $f = htmlentities($row['frequency']);
  $a = htmlentities($row['amortization']);
  $s = htmlentities($row['source']);
  $id = htmlentities($row['borrower_id']);
  ?>

  <div class="centered-edit">
    <?php echo "<h2>Edit $first_name's mortgage preferences</h2>"; ?>
  <form class="left-form" method="post">
    ...
    <p>
      ...
      <input type="number" id="price" name="price" value="<?php echo $pr ?>">
    </p>
    <p>
      <label for="closing_date">Closing/Refinance/Renewal Date</label>
      <input type="date" id="closing_date" name="closing_date" value="<?php echo $cd ?>">
    </p>
    <p>
      <label for="mortgage_amount">Mortgage Amount Requested</label>
      <input type="number" id="mortgage_amount" name="mortgage_amount" value="<?php echo $ma ?>">
    </p>
    <p>
    ...
    <p>
      ...
      <input type="number" id="amortization" name="amortization"
        placeholder="years" value="<?php echo $a ?>">
    </p>
    <p>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      $('#product').val('<?php echo $p ?>').change();
      $('#frequency').val('<?php echo $f ?>').change();
      $('#source').val('<?php echo $s ?>').change();
    });
  </script>
