...
<body>
  ...
  if (isset($_POST['borrower_id']) && isset($_POST['delete'])) {
    ...
    $sql = "DELETE FROM borrower_info WHERE borrower_id = :borrower_id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
  ...
  echo ' ';
  echo '<a class="button" href="borrower_info.php?borrower_id=' . htmlentities($row['borrower_id']) . '">Cancel</a>';
  echo '</p></form>';
  echo '</div>';
  echo '</div>';
  ?>
