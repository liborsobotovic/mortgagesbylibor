...
<body>
  ...
  if (isset($_POST['borrower_id']) && isset($_POST['debt_id']) &&
    isset($_POST['delete'])) {
    $bi = (int)$_POST['borrower_id'];
    $sql = "DELETE FROM liabilities WHERE borrower_id = :borrower_id AND
      debt_id = :debt_id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
  ...
  echo '<input type="hidden" name="debt_id"';
  echo 'value="' . htmlentities($row['debt_id']) . '">';
  echo '<p><input class="button" type="submit" name="delete" value="Delete">';
  echo ' ';
  echo '<a class="button" href="liabilities.php?borrower_id=' . htmlentities($row['borrower_id']) . '">Cancel</a>';
  echo '</p></form>';
  echo '</div>';
  echo '</div>';
  ?>
