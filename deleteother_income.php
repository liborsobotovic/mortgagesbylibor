...
<body>
  ...
  if (isset($_POST['borrower_id']) && isset($_POST['other_income_id']) &&
    isset($_POST['delete'])) {
    $bi = (int)$_POST['borrower_id'];
    $sql = "DELETE FROM other_income WHERE borrower_id = :borrower_id AND
      other_income_id = :other_income_id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
  ...
  echo '<input type="hidden" name="other_income_id"';
  echo 'value="' . htmlentities($row['other_income_id']) . '">';
  echo '<p><input class="button" type="submit" name="delete" value="Delete">';
  echo ' ';
  echo '<a class="button" href="other_income.php?borrower_id=' . htmlentities($row['borrower_id']) . '">Cancel</a>';
  echo '</p></form>';
  echo '</div>';
  echo '</div>';
  ?>
