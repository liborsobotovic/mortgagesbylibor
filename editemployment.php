...
<body>
  ...
  if(isset($_POST['employer']) && isset($_POST['address'])
    ...
    && isset($_POST['current']) && isset($_POST['bonus'])) {
    $bi = (int)$_POST['borrower_id'];
    if ($_POST['source'] == 'self-employed') {
      $_POST['income'] = null;
    };
    if ($_POST['current'] == 'No') {
      if ($_POST['end_date'] == '') {
    ...
    if($_POST['employer'] == '' || $_POST['address'] == ''
      ...
      || $_POST['current'] == '---SELECT---' || $_POST['bonus'] == '') {
        $_SESSION['error'] = "Fill out all the mandatory fields";
        header("Location: editemployment.php?borrower_id=$bi");
        return;
      }
    $sql = "UPDATE employment SET employer = :employer, address = :address,
      income_source = :income_source, occupation = :occupation,
      start_date = :start_date, current_source = :current_source,
      end_date = :end_date, income = :income, bonus = :bonus,
      type = :type, frequency = :frequency, borrower_id = :borrower_id
    ...
    header("Location: employment.php?borrower_id=$bi");
    return;
  }
  ...
  $e = htmlentities($row['employer']);
  $a = htmlentities($row['address']);
  $is = htmlentities($row['income_source']);
  $o = htmlentities($row['occupation']);
  $sd = htmlentities($row['start_date']);
  $cs = htmlentities($row['current_source']);
  $ed = htmlentities($row['end_date']);
  $i = htmlentities($row['income']);
  $b = htmlentities($row['bonus']);
  $t = htmlentities($row['type']);
  $f = htmlentities($row['frequency']);
  $id = htmlentities($row['borrower_id']);
  ?>

  <div class="centered-edit">
    <?php echo "<h2>Edit $first_name's employment information</h2>"; ?>
  <form class="left-form" method="post">
    <p>
      <label for="employer">Employer/Company Name</label>
      <input type="text" id="employer" name="employer" value="<?php echo $e ?>">
    </p>
    <p>
    ...
    <p>
      <label for="current">Current Source of Income?</label>
    ...
    <p class="center-p">
      ...
      <a class="button" href="employment.php?borrower_id=<?= htmlentities($_GET['borrower_id']) ?>">Cancel</a>
    </p>
  </form>
</div>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      $('#source').val('<?php echo $is ?>').change();
      $('#current').val('<?php echo $cs ?>').change();
      $('#type').val('<?php echo $t ?>').change();
      $('#frequency').val('<?php echo $f ?>').change();
    });
  </script>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      $('#current').change(function() {
        var selected_option = $('#current').val();
