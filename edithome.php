...
<body>
  ...
  if (isset($_POST['price']) && isset($_POST['balance'])
    ...
    if ($_POST['price'] == '' || $_POST['balance'] == ''
      ...
      || $_POST['condo'] == '---SELECT---') {
        ...
        header("Location: edithome.php?borrower_id=$bi");
        return;
      };
    $sql = "UPDATE home SET price = :price, value = :value,
      balance = :balance, lender = :lender, payments = :payments,
      maturity_date = :maturity_date, purchase_date = :purchase_date,
      taxes = :taxes, original_balance = :original_balance,
      rate_amount = :rate_amount, rate = :rate, amortization = :amortization,
      condo = :condo, condo_fee = :condo_fee, status = :status,
      borrower_id = :borrower_id WHERE borrower_id = :borrower_id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
      ":price" => $_POST['price'],
      ":value" => $_POST['value'],
  ...
  $v = htmlentities($row['value']);
  $b = htmlentities($row['balance']);
  $l = htmlentities($row['lender']);
  $pa = htmlentities($row['payments']);
  $md = htmlentities($row['maturity_date']);
  $pd = htmlentities($row['purchase_date']);
  $t = htmlentities($row['taxes']);
  $ob = htmlentities($row['original_balance']);
  $ra = htmlentities($row['rate_amount']);
  $r = htmlentities($row['rate']);
  $a = htmlentities($row['amortization']);
  ...
  <div class="centered-edit">
    <?php echo "<h2>Edit $first_name's existing home</h2>"; ?>
  <form class="left-form" method="post">
    ...
    <p>
      ...
      <input type="number" id="value" name="value" value="<?php echo $v ?>">
    </p>
    <p>
      <label for="balance">Current Mortgage Balance</label>
      <input type="number" id="balance" name="balance" value="<?php echo $b ?>">
    </p>
    <p>
      <label for="lender">Current Mortgage Lender</label>
      <input type="text" id="lender" name="lender" value="<?php echo $l ?>">
    </p>
    <p>
      <label for="payments">Monthly Payments</label>
      <input type="number" id="payments" name="payments" value="<?php echo $pa ?>">
    </p>
    <p>
      <label for="maturity_date">Current Maturity Date</label>
      <input type="date" id="maturity_date" name="maturity_date" value="<?php echo $md ?>">
    </p>
    <p>
      <label for="purchase_date">Date of Purchase</label>
      <input type="date" id="purchase_date" name="purchase_date" value="<?php echo $pd ?>">
    </p>
    <p>
    ...
    <p>
      ...
      <input type="number" id="original_balance" name="original_balance" value="<?php echo $ob ?>">
    </p>
    <p>
    ...
    <p>
      ...
      <input type="number" id="amortization" name="amortization" value="<?php echo $a ?>">
    </p>
    <p>
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      $('#rate_amount').val('<?php echo $ra ?>').change();
      $('#rate').val('<?php echo $r ?>').change();
      $('#condo').val('<?php echo $c ?>').change();
      $('#status').val('<?php echo $s ?>').change();
    });
  </script>
