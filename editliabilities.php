...
<body>
  ...
  if (isset($_POST['debt']) && isset($_POST['amount'])) {
    $bi = (int)$_POST['borrower_id'];
    $di = (int)$_POST['debt_id'];
    if ($_POST['debt'] == '---SELECT---' || $_POST['amount'] == '') {
      $_SESSION['error'] = "Fill out all the mandatory fields";
      header("Location: editliabilities.php?borrower_id=$bi&debt_id=$di");
      return;
    }

    $sql = "UPDATE liabilities SET debt = :debt, detail = :detail,
      amount = :amount, borrower_id = :borrower_id
      WHERE borrower_id = :borrower_id AND debt_id = :debt_id";
      $stmt = $pdo->prepare($sql);
      $stmt->execute(array(
        ":debt" => $_POST['debt'],
        ":detail" => $_POST['detail'],
  ...
  $sql = "SELECT * FROM liabilities WHERE borrower_id = :borrower_id AND
  debt_id = :debt_id";
  $stmt = $pdo->prepare($sql);
  $stmt->execute(array(
    ":borrower_id" => $_GET['borrower_id'],
  ...
  $d = htmlentities($row['debt']);
  $de = htmlentities($row['detail']);
  $am = htmlentities($row['amount']);
  $id = htmlentities($row['borrower_id']);
  ?>

  <div class="centered-edit">
    <?php echo "<h2>Edit $first_name's liabilities</h2>"; ?>
  <form class="left-form" method="post">
    ...
    <p>
      ...
      <textarea name="detail" rows="1" cols="40"><?php echo $de ?></textarea>
    </p>
    <p>
    ...
    <p class="center-p">
      ...
      <input type="hidden" name="debt_id"
        value="<?= htmlentities($_GET['debt_id']) ?>">
