...
<body>
  ...
  <?php echo "<h2>$first_name's other income</h2>"; ?>
  <table>
    <tbody id="mytab">
    </tbody>
  </table>
  ...
  <script type="text/javascript">
  $.getJSON('getother_income.php', function(rows) {
      $("#mytab").empty();
      found = false;
      for (var i = 0; i < rows.length; i++) {
          row = rows[i];
          found = true;
          $("#mytab").append('<h3>' + 'Income # ' + (i + 1) + '</h3>');
          ...
          if (row.income !== null) {
            $("#mytab").append('<tr><td>'
            + 'Income From This Source</td><td>'
            + "$" + Number(htmlentities(row.income)).toLocaleString("en")
            + '</td></tr>');
          }
          $("#mytab").append("<div class='buttons'><a class='edit-button' href='editother_income.php?borrower_id="
          + htmlentities(row.borrower_id) + "&other_income_id="
          + htmlentities(row.other_income_id) +"'>Edit</a>"
          + ' '
          + "<a class='delete-button' href='deleteother_income.php?borrower_id="
          + htmlentities(row.borrower_id) + "&other_income_id="
          + htmlentities(row.other_income_id) +"'>Delete</a></div>")
        }
      ...
      $("#options").empty();
      $("#options").append("<a class='button' href='addother_income.php?borrower_id=<?=$_GET['borrower_id']?>'>Add</a>");
  });
  </script>
