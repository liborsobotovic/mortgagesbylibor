...
<body>
  ...
  if (isset($_POST['asset1']) && isset($_POST['amount1'])) {
    $bi = (int)$_POST['borrower_id'];
    if ($_POST['asset1'] == '---SELECT---' || $_POST['amount1'] == '') {
      $_SESSION['error'] = "Fill out all the mandatory fields";
      header("Location: addasset.php?borrower_id=$bi");
      return;
    }
    for ($i = 1; $i <= 5; $i++) {
      if (! isset($_POST['asset'.$i])) continue;
      if (! isset($_POST['amount'.$i])) continue;
      $asset = $_POST['asset'.$i];
      $detail = $_POST['detail'.$i];
      $amount = $_POST['amount'.$i];

      $sql = "INSERT INTO assets (asset, detail, amount, borrower_id)
        VALUES (:asset, :detail, :amount, :borrower_id)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute(array(
          ":asset" => $asset,
          ":detail" => $detail,
  ...
  <form class="left-form" method="post">
    ...
    <p>
      <input class="button" type="button" id="more" value="Add asset">
    </p>
    <div id="next">
    </div>
    <p class="center-p">
      ...
      $("#more").click(function(event) {
        ...
        $('#next').append(
          '<div id="position'+count+'"> \
            <br><p><label for="asset">Asset</label> \
            <select id="asset" name="asset'+count+'"> \
              <option value="---SELECT---" selected>---SELECT---</option> \
              <option value="Savings">Savings</option> \
              <option value="Chequing">Chequing</option> \
              <option value="RRSP">RRSP</option> \
              <option value="Vehicle (year, make, model)">Vehicle (year, make, model)</option> \
              <option value="Investment (Stocks/Bonds/GIC/TFSA)">Investment (Stocks/Bonds/GIC/TFSA) \
              </option> \
              <option value="Other Assets">Other Assets</option> \
            </select> \
          </p> \
