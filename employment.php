...
<body>
  ...
  <?php echo "<h2>$first_name's employment information</h2>"; ?>
  <table>
    <tbody id="mytab">
    </tbody>
  </table>
  ...
  <script type="text/javascript">
  $.getJSON('getemployment.php', function(rows) {
      $("#mytab").empty();
      found = false;
      for (var i = 0; i < rows.length; i++) {
          ...
          if (row.current_source !== null) {
            $("#mytab").append('<tr><td>'
            + 'Current Source of Income</td><td>'
            + htmlentities(row.current_source)
            + '</td></tr>');
          }
          ...
          $("#options").empty();
          $("#options").append("<a class='edit-button' href='editemployment.php?borrower_id=<?php echo htmlentities($_GET['borrower_id']) ?>'>Edit</a> \
          <a class='delete-button' href='deleteemployment.php?borrower_id=<?php echo htmlentities($_GET['borrower_id']) ?>'>Delete</a>")
      }

      if ( ! found ) {
          $("#mytab").append("<tr><td>No entries found</td></tr>\n");
          $("#options").empty();
          $("#options").append("<a class='button' href='addemployment.php?borrower_id=<?=$_GET['borrower_id']?>'>Add</a>");
      }
  });
  </script>
