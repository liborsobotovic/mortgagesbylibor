...
<body>
  ...
  if (isset($_POST['borrower_id']) && isset($_POST['asset_id']) &&
    isset($_POST['delete'])) {
    $bi = (int)$_POST['borrower_id'];
    $sql = "DELETE FROM assets WHERE borrower_id = :borrower_id AND
      asset_id = :asset_id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
  ...
  echo '<input type="hidden" name="asset_id"';
  echo 'value="' . htmlentities($row['asset_id']) . '">';
  echo '<p><input class="button" type="submit" name="delete" value="Delete">';
  echo ' ';
  echo '<a class="button" href="assets.php?borrower_id=' . htmlentities($row['borrower_id']) . '">Cancel</a>';
  echo '</p></form>';
  echo '</div>';
  echo '</div>';
  ?>
