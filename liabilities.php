...
<body>
  ...
  $sql = "SELECT * FROM liabilities WHERE borrower_id = :borrower_id";
  $stmt = $pdo->prepare($sql);
  $stmt->execute(array(
  ...
  <?php echo "<h2>$first_name's liabilities</h2>"; ?>
  <table>
    <tbody id="mytab">
    </tbody>
  </table>
  ...
  <script type="text/javascript">
  $.getJSON('getliabilities.php', function(rows) {
      $("#mytab").empty();
      found = false;
      for (var i = 0; i < rows.length; i++) {
          row = rows[i];
          found = true;
          $("#mytab").append('<h3>' + 'Debt # ' + (i + 1) + '</h3>');
          if (row.debt !== null) {
            $("#mytab").append('<tr><td>'
            + 'Debt</td><td>'
            + htmlentities(row.debt)
            + '</td></tr>');
          }
          if (row.detail !== null) {
            $("#mytab").append('<tr><td>'
            + 'Description</td><td>'
            + htmlentities(row.detail)
            + '</td></tr>');
          }
          if (row.amount !== null) {
            $("#mytab").append('<tr><td>'
            + 'Amount</td><td>'
            + "$" + Number(htmlentities(row.amount)).toLocaleString("en")
            + '</td></tr>');
          }
          $("#mytab").append("<div class='buttons'><a class='edit-button' href='editliabilities.php?borrower_id="
          + htmlentities(row.borrower_id) + "&debt_id="
          + htmlentities(row.debt_id) +"'>Edit</a>"
          + ' '
          + "<a class='delete-button' href='deleteliabilities.php?borrower_id="
          + htmlentities(row.borrower_id) + "&debt_id="
          + htmlentities(row.debt_id) +"'>Delete</a></div>")
        }
      ...
      $("#options").empty();
      $("#options").append("<a class='button' href='addliability.php?borrower_id=<?=$_GET['borrower_id']?>'>Add</a>");
  });
  </script>
