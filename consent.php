...
<body>
  ...
  $sql = "SELECT * FROM consent WHERE borrower_id = :borrower_id";
  $stmt = $pdo->prepare($sql);
  $stmt->execute(array(
    "borrower_id" => $_GET['borrower_id']
  ));
  ...
  $sql = "INSERT INTO consent (consent, borrower_id) VALUES (:consent, :borrower_id)";
  if (isset($_POST['consent'])) {
    $bi = (int)$_POST['borrower_id'];
    $del = $pdo->prepare("DELETE FROM consent WHERE borrower_id = :borrower_id");
    $del->execute(array(
      "borrower_id" => $bi,
    ));
    if ($_POST['consent'] == "1") {
      ...
      header("Location: borrower.php?borrower_id=$bi");
      return;
    };
  ...
  <div class="main">
  <?php echo "<h2>$first_name, do you consent to a credit check?</h2>"; ?>
  <div class="add">
  <form class="left-form" method="post">
    <input type="hidden" name="consent" value="0">
    <label for="consent">Yes, I consent</label>
    <input type="checkbox" id="consent" name="consent" value="1" <?php echo isset($_SESSION['consent']) ? "checked" : ""; ?>>
    <input type="hidden" name="borrower_id"
      value="<?= htmlentities($_GET['borrower_id']) ?>">
    <p class="center-p">
      <input class="button" type="submit">
      <a class="button" href="borrower.php?borrower_id=<?= htmlentities($_GET['borrower_id']) ?>">Cancel</a>
    </p>
  </form>
</div>
</div>
