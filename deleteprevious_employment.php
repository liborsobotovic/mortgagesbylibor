...
<body>
  ...
  if (isset($_POST['borrower_id']) && isset($_POST['previous_employment_id']) &&
    isset($_POST['delete'])) {
    $bi = (int)$_POST['borrower_id'];
    $sql = "DELETE FROM previous_employer WHERE borrower_id = :borrower_id AND
      previous_employment_id = :previous_employment_id";
    $stmt = $pdo->prepare($sql);
    $stmt->execute(array(
      ":borrower_id" => $_POST['borrower_id'],
      ":previous_employment_id" => $_POST['previous_employment_id'],
    ));
  ...
  echo '<input type="hidden" name="previous_employment_id"';
  echo 'value="' . htmlentities($row['previous_employment_id']) . '">';
  echo '<p><input class="button" type="submit" name="delete" value="Delete">';
  echo ' ';
  echo '<a class="button" href="previous_employment.php?borrower_id=' . htmlentities($row['borrower_id']) . '">Cancel</a>';
  echo '</p></form>'
  echo '</div>';
  echo '</div>';
  ?>
