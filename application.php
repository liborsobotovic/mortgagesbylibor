...
<body>
  ...
  $.getJSON('getjson.php', function(rows) {
      ...
      for (var i = 0; i < rows.length; i++) {
          ...
          found = true;
          $("#mytab").append("<tr><td>"
              + '<a class="link" href="borrower.php?borrower_id='
              + htmlentities(row.borrower_id) +'">'
              + htmlentities(row.first_name) + " " + htmlentities(row.last_name)
              + '</a></td><td>'
              + '<a class="edit-button" href="edit.php?borrower_id='
              + htmlentities(row.borrower_id) +'">'
              + 'Edit</a> '
              + '<a class="delete-button" href="delete.php?borrower_id='
              + htmlentities(row.borrower_id) +'">'
              + 'Delete</a>\n</td></tr>');
      }
      if ( ! found ) {
  ...
  <div class="center-link">
    <a class="button" href="add.php">Add a borrower</a>
  </div>
</div>
