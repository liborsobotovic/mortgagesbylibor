...
<body>
  ...
  if (isset($_POST['address']) && isset($_POST['occupancy'])
    ...
    if ($_POST['address'] == '' || $_POST['occupancy'] == '---SELECT---'
      ...
      || $_POST['condo'] == '---SELECT---') {
        ...
        header("Location: editemployment.php?borrower_id=$bi");
        return;
      };
    $sql = "UPDATE property SET address = :address, occupancy = :occupancy,
      rental = :rental, price = :price, taxes = :taxes, heating = :heating,
      condo = :condo, condo_fee = :condo_fee, dwelling_type = :dwelling_type,
      dwelling_style = :dwelling_style, garage = :garage, age = :age,
      heat = :heat, living_space = :living_space, living_number = :living_number,
      lot_size = :lot_size, lot_number = :lot_number, water = :water,
      sewage = :sewage, borrower_id = :borrower_id
  ...
  $o = htmlentities($row['occupancy']);
  $r = htmlentities($row['rental']);
  $p = htmlentities($row['price']);
  $t = htmlentities($row['taxes']);
  $he = htmlentities($row['heating']);
  $c = htmlentities($row['condo']);
  $cf = htmlentities($row['condo_fee']);
  $dt = htmlentities($row['dwelling_type']);
  $ds = htmlentities($row['dwelling_style']);
  $g = htmlentities($row['garage']);
  $ag = htmlentities($row['age']);
  $h = htmlentities($row['heat']);
  $lis = htmlentities($row['living_space']);
  $lin = htmlentities($row['living_number']);
  $ls = htmlentities($row['lot_size']);
  $ln = htmlentities($row['lot_number']);
  $w = htmlentities($row['water']);
  $s = htmlentities($row['sewage']);
  $id = htmlentities($row['borrower_id']);
  ?>

  <div class="centered-edit">
    <?php echo "<h2>Edit $first_name's property to be mortgaged</h2>"; ?>
  <form class="left-form" method="post">
    <p>
      <label for="address">Address</label>
      <textarea name="address" rows="2" cols="40"
        placeholder="Street Number, Street Name&#10;City, Province, Postal Code"><?php echo $a ?></textarea>
    </p>
    <p>
    ...
    <p>
      ...
      <input type="number" id="price" name="price" value="<?php echo $p ?>">
    </p>
    <p>
      <label for="taxes">Annual Property Taxes</label>
      <input type="number" id="taxes" name="taxes" value="<?php echo $t ?>">
    </p>
    <p>
      <label for="heating">Monthly Heating Cost</label>
      <input type="number" id="heating" name="heating" value="<?php echo $he ?>">
    </p>
    <p>
    ...
    <p>
      ...
      <input type="number" id="age" name="age" value="<?php echo $ag ?>">
    </p>
    <p>
    ...
    <p>
      ...
      <input type="number" id="living_space" name="living_space" value="<?php echo $lis ?>">
    ...
    <p>
      ...
      <input type="number" id="lot_size" name="lot_size" value="<?php echo $ls ?>">
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      $('#occupancy').val('<?php echo $o ?>').change();
      $('#condo').val('<?php echo $c ?>').change();
      $('#dwelling_type').val('<?php echo $dt ?>').change();
      $('#dwelling_style').val('<?php echo $ds ?>').change();
      $('#garage').val('<?php echo $g ?>').change();
      $('#heat').val('<?php echo $h ?>').change();
      $('#living_number').val('<?php echo $lin ?>').change();
      $('#lot_number').val('<?php echo $ln ?>').change();
      $('#water').val('<?php echo $w ?>').change();
      $('#sewage').val('<?php echo $s ?>').change();
    });
  </script>

  <script type="text/javascript">
    $(document).ready(function() {
      if ($('#occupancy').val() == 'Owner-Occupied & Rental' || $('#occupancy').val() == 'Rental') {
        $('#rental_income').empty();
        $('#rental_income').append(
          '<label for="rental">Rental Income</label> \
            <input type="number" id="rental" name="rental" value="<?php echo $r ?>">').show();
        } else {
          $('#rental_income').empty();
      };
    });
          ...
          if($('#condo').val() == 'Yes') {
            ...
            $('#condo_fees').append(
              '<label for="condo_fee">Monthly Condo Fees</label> \
                <input type="number" id="condo_fee" name="condo_fee" value="<?php echo $cf ?>">').show();
            } else {
              $('#condo_fees').empty();
            };
          });
